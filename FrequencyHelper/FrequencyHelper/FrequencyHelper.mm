...
//  FrequencyMeter
//
//  Created by Valentin Strazdin on 1/2/22.
//

#import "FrequencyHelper.h"
...
@implementation FrequencyHelper
...
void AudioCallback( Float32 * buffer, UInt32 frameSize, void * userData )
{
    ...
    if (accumulateFrames(buffer, frameSize)==YES) { //if full
        ...
        NSString *maxHZString = [NSString stringWithFormat:@"%0.3f", maxHZ];
        dispatch_async(dispatch_get_main_queue(), ^{ //update UI only on main thread
            [[NSNotificationCenter defaultCenter] postNotificationName:kFREQUENCY_DETECTED
                                                                object:nil
                                                              userInfo:@{kMAX_HZ_KEY: maxHZString}];
        });
